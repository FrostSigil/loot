...
module.exports = function AutoLootOld(mod) {
	...
	cmd.add('loot', {
		...
		m() {
			myst = !myst;
			mod.command.message(`Лут 2 phaze ${myst ? "en" : "dis"}abled`);
		}
	});
    ...
    mod.hook('S_SPAWN_DROPITEM', 8, (e) => {
		if(!(config.blacklist.includes(e.item)) && (((e.item < 8000 || e.item > 8017) && myst && mod.game.me.inCombat) || (e.item < 8000 || e.item > 8024)) && e.owners.some(owner => owner === mod.game.me.playerId)){
